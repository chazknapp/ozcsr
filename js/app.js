...
if (window.MapboxGeocoder) {
  ...
  try {
    slot.appendChild(geocoder.onAdd(map));
  } catch (e) {
    // Fallback: mount as a map control
...
function recenterMap() {
  ...
  catch (_) { map.flyTo({ center: START_CENTER, zoom: START_ZOOM }); }
...
// Zoom buttons wiring — zoom toward the address marker if present
function zoomToPoint(delta) {
  const current = map.getZoom();
  const target = Math.max(0, Math.min(22, current + delta));
  let center = map.getCenter();
  if (addrMarker && typeof addrMarker.getLngLat === 'function') {
    try { center = addrMarker.getLngLat(); } catch(_) {}
  }
...
const ZONE_PALETTE = [
  // 20 high-contrast categorical colors (Vega/"Tableau 20" inspired)
  "#1f77b4", // blue
  "#ff7f0e", // orange
  "#2ca02c", // green
  "#d62728", // red
...
map.on("load", async () => {
  ...
  // Add interactivity (click/hover popups)
  addInteractivity();
...
function addLayers() {
  ...
  // FEEDERS: show as filled polygons (same palette as zones) + yellow outline
  if (map.getSource("feeders-src")) {
    ...
    if (feedersHasPolygons) {
      map.addLayer({
        ...
        source: "feeders-src",
        paint: {
          // reuse the per-substation color mapping
          "fill-color": zonesFillColorExpr,
    ...
    map.addLayer({
      ...
      source: "feeders-src",
      paint: {
        "line-color": "#fff3b0",   // soft white-yellow
        "line-width": 1.2,          // thin
        "line-opacity": 0.7         // pastel
      },
  ...
  // GRIDS (InT): deep yellow border, no fill + labels
  if (map.getSource("gridInT-src")) {
    map.addLayer({ id: "gridInT-line", type: "line", source: "gridInT-src", paint: { "line-color": "#f59e0b", "line-width": 1.6 } });
...
function addInteractivity() {
  ...
  if (map.getLayer("gridInT-line")) {
    map.on("click", "gridInT-line", (e) => {
      const f = e.features && e.features[0];
      const code = f?.properties?.[FIELDS.gridInT.sml] ?? "?";
      new mapboxgl.Popup().setLngLat(e.lngLat).setHTML(`<b>Grid</b>: ${code}`).addTo(map);
    });
  }
  if (map.getLayer("zones-fill")) {
    map.on("click", "zones-fill", (e) => {
      ...
      new mapboxgl.Popup().setLngLat(e.lngLat).setHTML(`<b>Substation</b>: ${name}`).addTo(map);
    });
  }
  // Feeder hover should work anywhere over the polygon OR the outline.
  // Instead of per-layer handlers, use a single map-level query so feeds win over zones.
...
if (geocoder) geocoder.on("result", (e) => {
  ...
  // Marker
  if (!addrMarker) addrMarker = new mapboxgl.Marker();
  addrMarker.setLngLat([lng, lat]).addTo(map);
  map.flyTo({ center: [lng, lat], zoom: 15 });
  // after the map re-renders, punch a hole in the feeder polygon under the address
  ...
  if (FIELDS.substations.type === "polygon") {
    const hit = Spatial.polygonContainingPoint(addrPoint, data.substations);
    substationName = hit ? (hit.properties?.[FIELDS.substations.name] ?? "—") : "—";
  } else { // point
  ...
  if (FIELDS.feeders.type === "line") {
    ...
    console.log('Nearest feeder result:', fRes);
    if (fRes.feature) {
      ...
      // highlight the chosen feeder so we can see what was picked
      try {
    ...
    const fHit = Spatial.polygonContainingPoint(addrPoint, data.feeders);
    feederCode = fHit ? (getFeederCodeFromProps(fHit.properties) ?? "—") : "—";
    feederSubstation = fHit ? (getFeederSubstationFromProps(fHit.properties) ?? "—") : "—";
    feederDistText = fHit ? "inside / assigned" : "—";
  }
  ...
  if (gridSML != null) {
    gridSMLText = `${gridSML}`;
    gridOOTText = "This is In Territory";
  } else {
    gridSMLText = "This is OOT";
    if (data.gridOOT) {
      const nearOOT = Spatial.nearestPolygonByEdge(addrPoint, data.gridOOT);
      gridOOTText = nearOOT.feature ? `${nearOOT.feature.properties?.[FIELDS.gridInT.sml] ?? "—"}` : "—"; // uses Number_ when OOT layer added
    } else {
      gridOOTText = "—"; // OOT layer not yet provided
    }
  }

  // Scheduling helper: nearest few grids (optional)
  let neighborHtml = "";
  if (SHOW_NEIGHBOR_GRIDS && data.gridInT) {
    const neighbors = Spatial.nearestPolygonsByCentroid(addrPoint, data.gridInT, NEIGHBOR_GRID_COUNT);
    if (neighbors.length) {
      neighborHtml = `<div class="label" style="margin-top:6px">Nearby Grids</div>` +
        neighbors.map(n => {
          const code = n.feature.properties?.[FIELDS.gridInT.sml] ?? "?";
          const bearing = Math.round(((n.bearing % 360) + 360) % 360); // 0..359
          const dir = bearingToCardinal(bearing);
          return `<div class="small">${code} • ${n.distanceMiles.toFixed(2)} mi • ${dir}</div>`;
        }).join("");
    }
  }
  ...
  document.getElementById("feeder-val").textContent = feederCode;
...
// Helper: create a green square icon for huts
async function addHutSquareIcon() {
  ...
  try { map.addImage('hut-square', imgData, { pixelRatio: 2 }); } catch(_) {}
}

// Helper: bearing to cardinal
